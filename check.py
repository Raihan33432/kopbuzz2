...
if __name__ == "__main__":
    # 1) Set the webhook (must await)
    print("Setting webhook to:", WEBHOOK_URL)
    asyncio.run(application.bot.set_webhook(WEBHOOK_URL))

    # 2) Run the built-in webhook server
    application.run_webhook(
        listen="0.0.0.0",
        port=PORT,
        url_path=WEBHOOK_PATH,       # <-- changed from webhook_path
        webhook_url=WEBHOOK_URL,     # <-- pass the external URL
